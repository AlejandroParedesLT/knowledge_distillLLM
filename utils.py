...
def get_model(args, device: int) -> PreTrainedModel:
    ...
    st_time = time.time()
    dtype = torch.float16 #eval("cuda")#eval(args.dtype)
    if args.model_parallel:
        ...
        model = AutoModelForCausalLM.from_pretrained(args.model_path, config=config, device_map={"": f"cuda:{device}"}, torch_dtype=dtype)
