...
# srun bash -c "source \$HOME/minillm/.env && source \$VENV_DIR/bin/activate && huggingface-cli login --token \$HF_TOKEN && bash scripts/qwen2.5/sft/sft_1.5B.sh"

# 2. Perform a KD on the Qwen2.5-0.5B student model using the Qwen2.5-1.5B teacher model
srun bash -c "source \$HOME/minillm/.env && source \$VENV_DIR/bin/activate && huggingface-cli login --token \$HF_TOKEN && bash scripts/qwen2.5/kd/kd_1.5B_0.5B.sh"
