...
#srun bash -c "source \$HOME/minillm/.env && source \$VENV_DIR/bin/activate && huggingface-cli login --token \$HF_TOKEN && bash scripts/gpt2/sft/sft_xlarge_pytorrent.sh"
srun bash -c "source \$HOME/minillm/.env && source \$VENV_DIR/bin/activate && huggingface-cli login --token \$HF_TOKEN && bash scripts/gpt2/sft/sft_base_pytorrent.sh"


#srun bash -c "source \$HOME/minillm/.env && source \$VENV_DIR/bin/activate && huggingface-cli login --token \$HF_TOKEN && bash scripts/gpt2/kd/kd_base_pytorrent.sh"
